...
// The VPN service is injected by the caller (constructed in cmd) so its
// lifecycle is owned by the entrypoint, not silently created here.
func NewRouter(cfg *config.Config, db *database.DB, vpnSvc *vpn.Service) *chi.Mux {
	...
	if vpnSvc != nil && !vpnSvc.HasAWGStatusProvider() {
...
func NewRouterWithOptions(opts Options) *chi.Mux {
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/users", func(r chi.Router) {
			...
			r.Patch("/{user_id}", h.UpdateUserHandler)
			r.Post("/{user_id}/delete", h.DeleteUserHandler)
			r.Delete("/{user_id}", h.DeleteUserHandler)
