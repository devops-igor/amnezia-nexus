...
// performClientHandshakeUnconfirmed completes initiator-side response verification
// without sending the first authenticated transport packet. The responder therefore
// remains in current+next state until the caller confirms next.
...
func confirmClientHandshake(
	...
	// Upstream initiators immediately send an authenticated keepalive after
	// consuming the response. That first transport packet confirms responder next.
...
// authenticated keepalive used by upstream initiators to confirm responder next.
